...
def parse_args(args):
    ...
    parser.add_argument('--path_to_result',
                        help='Path to the folder and filename where the result of segmentation should be saved. '
                             'Default is: test_outcome.png',
                        default='test_outcome.png')
...
def post_image():
    ...
    img_input = Input(shape=input_shape) # use to create Keras tensor. return tensor object
    x = create_tiramisu(number_classes, img_input)
    model = Model(img_input, x)
    model.load_weights(args.path_to_model)

    try_image = img.resize((input_shape[0], input_shape[1]), Image.NEAREST)
    # try_image.show()
    ...
    return send_file(args.path_to_result, mimetype='image/gif')
